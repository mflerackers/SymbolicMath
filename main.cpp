...
void matrixTests() {
  ...
  auto m = matrix({vec({constant(1.0f), constant(0.0f)}),
                   vec({constant(0.0f), constant(1.0f)})});
  std::cout << "matrix " << m << "\n";

  m = matrix({vec({cos(x), -sin(x)}), vec({sin(x), cos(x)})});
  std::cout << "matrix " << m << "\n";

  m = matrix({vec({constant(1.0f), constant(0.0f)}),
              vec({constant(0.0f), constant(1.0f)})}) +
      matrix({vec({constant(2.0f), constant(4.0f)}),
              vec({constant(6.0f), constant(8.0f)})});
  std::cout << "matrix sum " << m << "\n";
  std::cout << "matrix sum " << m.simplify() << "\n";

  m = matrix({vec({constant(3.0f), constant(0.0f)}),
              vec({constant(0.0f), constant(2.0f)})}) *
      matrix({vec({constant(2.0f), constant(0.0f)}),
              vec({constant(0.0f), constant(3.0f)})});
  std::cout << "matrix product " << m << "\n";
  std::cout << "matrix product " << m.simplify() << "\n";

  m = matrix({vec({cos(x), -sin(x)}), vec({sin(x), cos(x)})}) *
      matrix({vec({cos(x), sin(x)}), vec({-sin(x), cos(x)})});
  std::cout << "matrix product " << m << "\n";
  std::cout << "matrix product " << m.simplify() << "\n";

  m = matrix({vec({cos(x), -sin(x)}), vec({sin(x), cos(x)})}) *
      matrix({vec({2.0f, 0.0f}), vec({0.0f, 2.0})}) *
      matrix({vec({cos(x), sin(x)}), vec({-sin(x), cos(x)})});
  std::cout << "cov matrix product " << m << "\n";
  m = m.simplify();
  std::cout << "cov matrix product " << m << "\n";
  ...
  m = matrix({vec({cos(x), -sin(x)}), vec({sin(x), cos(x)})}) *
      matrix({vec({1.0f, 0.0f}), vec({0.0f, 2.0})}) *
      matrix({vec({cos(x), sin(x)}), vec({-sin(x), cos(x)})});
  std::cout << "cov matrix product " << m << "\n";
  m = m.simplify();
  std::cout << "cov matrix product " << m << "\n";
...
int main() {
  ...
  auto y = x;
  std::cout << "expression " << y << "\n";
  auto iy = y.integrate().simplify();
  std::cout << "integral " << iy << "\n";
  auto diy = iy.derive().simplify();
  std::cout << "derivative " << diy << "\n";

  auto yy = 8 * (x ^ 3);
  std::cout << "expression " << yy << "\n";
  auto iyy = yy.integrate().simplify();
  std::cout << "integral " << iyy << "\n";
